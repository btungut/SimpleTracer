...
namespace SimpleTracer.UnitTests.Internal
{
    ...
    public class SubscriptionHandlerTests
    {
        ...
        public void Start_WithoutEnqueuedItem_TaskWillBeTriggered()
        {
            ...
            Assert.True(WaitUntil(() => toBeFilled != null, TimeSpan.FromSeconds(3)));
        ...
        public void Start_EnqueuedCountMoreThanExecutionCapacity_NotificationsShouldNotExceedTheExecutionCapacity()
        {
            ...
            //Wait until all enqueued items are dequeued and added the notifications list to assert.
            Assert.True(WaitUntil(() => handler.Queue.Count == 0, TimeSpan.FromSeconds(3)));
        ...
        public void Start_CancelledTask_NewTaskShouldBeCreated()
        {
            ...
            handler.Start(sts);
            Assert.False(WaitUntil(() => count > 0, TimeSpan.FromSeconds(3)));

            handler.Start();
            Assert.True(WaitUntil(() => count > 0, TimeSpan.FromSeconds(3)));
        }

        [Fact]
        public void Start_ThrowingExceptionInExecution_ExceptionShouldNotBeThrowedToCaller()
        {
            int toBeWaitedCount = 10;
            List<IEventNotification> notifications = new List<IEventNotification>();
            Func<IEventNotification, Task> func = @event =>
            {
                notifications.Add(@event);
            ...
            Assert.True(WaitUntil(() => notifications.Count == toBeWaitedCount, TimeSpan.FromSeconds(3)));
        }
        ...
        public void Start_ValidStatement_DistanceBetweenLastExecutionsShouldBeAsMuchAsDelay()
        {
            ...
            {
                var distance = notifications[i + 1].LastExecution - notifications[i].LastExecution;
                Assert.True(distance >= subscription.Execution.Interval);
            }
        }
        ...
        public void Start_AfterDisposing_NoMoreNotificationShouldBeTriggered()
        {
            ...
            //Wait until all enqueued items are dequeued and added the notifications list to assert.
            WaitUntil(() => notifications.Count == toBeWaitedCount, TimeSpan.FromSeconds(3));

            //Wait until specified timeout is up. Any notification shouldn't be added and statement should be false.
            Assert.False(WaitUntil(() => notifications.Count > toBeWaitedCount, TimeSpan.FromSeconds(3)));
        }
