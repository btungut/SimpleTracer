...
namespace SimpleTracer.AcceptanceTests
{
    public abstract class BaseAcceptanceTest
    {
        ...
        protected bool CompareListsOfWrittenAndListenedEvents(IEnumerable<WrittenDummyEvent> writtenEvents, IEnumerable<IEventEntry> listenedEvents)
        {
            if (writtenEvents.Count() != listenedEvents.Count())
                return false;

            var writtenEventsAsOrdered = writtenEvents.Select(o => o.EventId).OrderBy(o => o);
            var listenedEventsAsOrdered = listenedEvents.Select(o => o.Id).OrderBy(o => o);

            return writtenEventsAsOrdered.SequenceEqual(listenedEventsAsOrdered);
        }
    }
    ...
    public class DummyEventSource : EventSource
    {
        ...
        public ConcurrentBag<WrittenDummyEvent> WrittenEvents = new ConcurrentBag<WrittenDummyEvent>();
